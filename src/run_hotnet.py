...
sys.path.insert(0, 'C:\\Users\\yinyin\\Desktop\\Project\\GDSC-drug-modules\\src')
...
def single_hotnet(score_file,index_file,similarity_matrix,perm,output_folder):
    ...
    pelf = [hierarchies_permuted_folder + "/edge_list_%d.tsv" % i for i in list(range(perm))]
    pigf = [hierarchies_permuted_folder + "/index_gene_%d.tsv" % i for i in list(range(perm))]
...
if __name__=='__main__':
    ...
    print('step3')
    for netfold in args.network_folders:
        print('single hot {}'.format(netfold))
        for scorefile in args.scores_files:
            dirname = get_dirname(netfold, scorefile)
